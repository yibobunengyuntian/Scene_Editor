...
void ImageItem::wheelEvent(QGraphicsSceneWheelEvent *event)//鼠标滚轮事件
{
...
void ImageItem::setQGraphicsViewWH(int nwidth, int nheight)//将主界面的控件QGraphicsView的width和height传进本类中，并根据图像的长宽和控件的长宽的比例来使图片缩放到适合控件的大小
{
...
void ImageItem::ResetItemPos()//重置图片位置
{
