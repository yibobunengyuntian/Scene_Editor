    ...
    QWidget(parent)
{
    ...
    speedBox->setToolTip(QStringLiteral("相机移动速度"));
    xBox->setToolTip(QStringLiteral("相机位置 X 坐标"));
    yBox->setToolTip(QStringLiteral("相机位置 Y 坐标"));
    zBox->setToolTip(QStringLiteral("相机位置 Z 坐标"));
    resBtn->setStyleSheet(m_styleSheet);
    resBtn->setToolButtonStyle(Qt::ToolButtonIconOnly);
    resBtn->setIcon(QIcon(":/res/icon/res.png"));
    resBtn->setToolTip(QStringLiteral("相机复位"));
